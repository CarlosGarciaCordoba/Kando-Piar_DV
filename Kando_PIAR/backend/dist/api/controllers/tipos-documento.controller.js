...
exports.TiposDocumentoController = void 0;
...
class TiposDocumentoController {
    ...
    static async createTipoDocumento(req, res) {
        try {
            const { codigo, descripcion, activo = true, orden = 1, observaciones } = req.body; // Validaciones básicas
            ...
            const existingResult = await database_1.default.query(checkQuery, [codigo]);
    ...
    static async updateTipoDocumento(req, res) {
        try {
            const { id } = req.params;
            const { codigo, descripcion, activo, orden, observaciones } = req.body;
            // Validar UUID
            const uuidRegex = /^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i;
            if (!id || !uuidRegex.test(id)) {
                return res.status(400).json({
            ...
            const existsResult = await database_1.default.query(checkExistsQuery, [id]);
            ...
            const duplicateResult = await database_1.default.query(checkDuplicateQuery, [codigo, id]);
    ...
    static async deleteTipoDocumento(req, res) {
        try {
            ...
            const existsResult = await database_1.default.query(checkQuery, [id]);
            if (existsResult.rows.length === 0) {
                return res.status(404).json({
                    success: false,
                    message: 'Tipo de documento no encontrado'
            ...
            const deleteResult = await database_1.default.query(deleteQuery, [id]);
...
exports.TiposDocumentoController = TiposDocumentoController;
