...
exports.login = exports.resetPassword = exports.recoverPassword = void 0;
const bcryptjs_1 = __importDefault(require("bcryptjs"));
const jsonwebtoken_1 = __importDefault(require("jsonwebtoken"));
const database_1 = __importDefault(require("../../config/database"));
const gmailSender_1 = require("../../utils/gmailSender");
...
const recoverPassword = async (req, res) => {
    try {
        ...
        const userResult = await database_1.default.query('SELECT nombres, apellidos FROM usuarios WHERE email = $1', [email]);
        ...
        await (0, gmailSender_1.sendGmail)(email, subject, html);
...
const resetPassword = async (req, res) => {
    try {
        ...
        const match = await bcryptjs_1.default.compare(token, record.tokenHash);
        if (!match) {
            return res.status(400).json({ success: false, message: 'Token inválido' });
        }
        // Obtener usuario por email
        const userResult = await database_1.default.query('SELECT cedula FROM usuarios WHERE email = $1', [record.email]);
        const user = userResult.rows[0];
        if (!user) {
            return res.status(404).json({ success: false, message: 'Usuario no encontrado' });
        }
        const newHash = await bcryptjs_1.default.hash(newPassword, 10);
        await database_1.default.query('UPDATE usuarios SET password_hash = $1, debe_cambiar_password = false WHERE cedula = $2', [newHash, user.cedula]);
        record.used = true;
...
const login = async (req, res) => {
    try {
        ...
        const userResult = await database_1.default.query(`SELECT cedula, codigo_usuario, nombres, apellidos, email, telefono, 
                    password_hash, codigo_institucion, debe_cambiar_password, 
                    estado, intentos_fallidos, bloqueado_hasta 
             FROM usuarios 
             WHERE codigo_usuario = $1 AND codigo_institucion = $2 AND estado = true`, [userCode, institution]);
        const user = userResult.rows[0];
        ...
        if (!validPassword) {
            ...
            const bloqueado_hasta = intentos >= 3 ?
                ...
                null;
            await database_1.default.query('UPDATE usuarios SET intentos_fallidos = $1, bloqueado_hasta = $2 WHERE cedula = $3', [intentos, bloqueado_hasta, user.cedula]);
            return res.status(401).json({
                success: false,
                message: 'Código de usuario, contraseña o código de institución incorrectos'
            });
        ...
        }, process.env.JWT_SECRET || 'tu_clave_secreta_kando', { expiresIn: '8h' });
